(function(){
  ...
  function bindForm(){
    ...
    $('#aiCompleteForm')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget;const f=new FormData(form);setBusy(form,true);try{result((await api('complete_task',{title:String(f.get('title')),description:String(f.get('description')),taskType:String(f.get('taskType')),user:currentUserName()})).text)}catch(err){error(err)}finally{setBusy(form,false)}});
  }
